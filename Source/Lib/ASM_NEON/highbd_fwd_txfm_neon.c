...
static INLINE void transpose4x_N4(const int32x4_t *in, int32x4_t *out) {
...
static INLINE void write_buffer_8x8_N4(int32x2_t in0, int32x2_t in1, int32_t *out) {
    ...
    memset(out + 2, 0, 6 * sizeof(int32_t));
    memset(out + 10, 0, (6 + 6 * 8) * sizeof(int32_t));
}
...
static void highbd_fdct16_x4_N4_neon(const int32x4_t *in, int32x4_t *out, int bit) {
    const int32_t *const cospi = cospi_arr_s32(bit);
    const int32x4_t      v_bit = vdupq_n_s32(-bit);
...
static void highbd_fadst16_x4_N4_neon(const int32x4_t *in, int32x4_t *out, int bit) {
    const int32_t *const cospi = cospi_arr_s32(bit);
    const int32x4_t      v_bit = vdupq_n_s32(-bit);
...
static void highbd_fidentity16_x4_N4_neon(const int32x4_t *in, int32x4_t *out, int bit) {
    (void)bit;
    const int32x4_t fact = vdupq_n_s32(2 * new_sqrt2);
...
static void highbd_fdct16_xn_N4_neon(const int32x4_t *in, int32x4_t *out, int bit, const int howmany) {
    const int stride = 16;
    int       i      = 0;
    do { highbd_fdct16_x4_N4_neon(in + i * stride, out + i * stride, bit); } while (++i < howmany);
}

static void highbd_fadst16_xn_N4_neon(const int32x4_t *in, int32x4_t *out, int bit, int howmany) {
    const int stride = 16;
    int       i      = 0;
    do { highbd_fadst16_x4_N4_neon(in + i * stride, out + i * stride, bit); } while (++i < howmany);
}

static void highbd_fidentity16_xn_N4_neon(const int32x4_t *in, int32x4_t *out, int bit, int howmany) {
    const int stride = 16;
    int       i      = 0;
    do { highbd_fidentity16_x4_N4_neon(in + i * stride, out + i * stride, bit); } while (++i < howmany);
}

static INLINE void transpose16x4_N4(const int32x4_t *in, int32x4_t *out) {
    transpose_elems_s32_4x4(in[0], in[1], in[2], in[3], &out[0], &out[1], &out[2], &out[3]);
...
static INLINE void load_4x4_in_16x16(const int16_t *input, int32x4_t *buf, int stride) {
...
static INLINE void load_4x4_in_16x16_flip(const int16_t *input, int32x4_t *buf, int stride) {
...
void svt_av1_fwd_txfm2d_16x16_N4_neon(int16_t *input, int32_t *output, uint32_t stride, TxType tx_type, uint8_t bd) {
    ...
    case V_DCT:
        ...
        break;
    case H_DCT:
        load_4x4_in_16x16(input, buf0, stride);
    ...
    case V_FLIPADST:
        ...
        break;
    case H_FLIPADST:
        load_4x4_in_16x16_flip(input, buf0, stride);
        highbd_fidentity16_xn_N4_neon(buf0, buf0, fwd_cos_bit_col[2][2], 4);
        shift_right_2_round_s32_x4_N4(buf0, buf0);
        transpose16x4_N4(buf0, buf1);
        highbd_fadst16_x4_N4_neon(buf1, buf1, fwd_cos_bit_row[2][2]);
...
static void highbd_fdct32_x4_N4_neon(const int32x4_t *input, int32x4_t *output, int cos_bit) {
    const int32_t *const cospi     = cospi_arr_s32(cos_bit);
    const int32x4_t      v_cos_bit = vdupq_n_s32(-cos_bit);
...
static void highbd_fidentity32_x4_N4_neon(const int32x4_t *input, int32x4_t *output, int cos_bit) {
    (void)cos_bit;
    for (int i = 0; i < 8; i++) { output[i] = vshlq_n_s32(input[i], 2); }
}

static void highbd_fdct32_xn_N4_neon(const int32x4_t *in, int32x4_t *out, int bit, const int howmany) {
    const int stride = 32;
    int       i      = 0;
    do { highbd_fdct32_x4_N4_neon(in + i * stride, out + i * stride, bit); } while (++i < howmany);
}

static void highbd_fidentity32_xn_N4_neon(const int32x4_t *in, int32x4_t *out, int bit, int howmany) {
    const int stride = 32;
    int       i      = 0;
...
static INLINE void transpose32x8_N4(const int32x4_t *in, int32x4_t *out) {
    for (int i = 0; i < 8; i++) {
...
static INLINE void transpose8x8_N4(const int32x4_t *in, int32x4_t *out) {
    transpose_elems_s32_4x4(in[0], in[1], in[2], in[3], &out[0], &out[1], &out[2], &out[3]);
...
static void highbd_fdct64_x4_N4_neon(const int32x4_t *input, int32x4_t *output, int8_t cos_bit) {
    const int32_t *const cospi     = cospi_arr_s32(cos_bit);
    const int32x4_t      v_cos_bit = vdupq_n_s32(-cos_bit);
...
static void highbd_fdct64_xn_N4_neon(const int32x4_t *in, int32x4_t *out, int bit, const int howmany) {
    const int stride = 64;
    int       i      = 0;
    do { highbd_fdct64_x4_N4_neon(in + i * stride, out + i * stride, bit); } while (++i < howmany);
}

static void highbd_fidentity64_xn_N4_neon(const int32x4_t *in, int32x4_t *out, int bit, int howmany) {
    const int stride = 64;
    int       i      = 0;
...
static INLINE void transpose64x16_N4(const int32x4_t *in, int32x4_t *out) {
    for (int i = 0; i < 16; i++) {
...
static INLINE void transpose16x16_N4(const int32x4_t *in, int32x4_t *out) {
    for (int i = 0; i < 4; i++) {
...
static INLINE void load_buffer_16x16_in64x64(const int16_t *input, int32x4_t *buf, int stride) {
